...
Copy item link asks Mail or Apple Notes for the identifiers of the items you selected. Mail message IDs are converted directly to `message:` links. For Notes, Vorssaint looks up only each selected note’s stable identifier in the local Notes database and converts it to an `applenotes:` link; it does not read note titles, bodies or attachments. The resulting links go only to the system clipboard and are never uploaded.
